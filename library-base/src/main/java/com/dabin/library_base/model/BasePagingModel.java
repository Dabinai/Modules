...
public abstract class BasePagingModel<T> extends SuperBaseModel<T> {
            ...
            mUiHandler.postDelayed(() -> {
                for (WeakReference<IBaseModelListener> weakListener : mWeakReferenceDeque) {
                    if (weakListener.get() instanceof IPagingModelListener) {
                        IPagingModelListener iModelListener = (IPagingModelListener) weakListener.get();
                        if (null != iModelListener) {
                            iModelListener.onLoadFinish(BasePagingModel.this, data, isEmpty, isFirstPage);
                        }
                    }
                }
     ...
     */
    public void loadFail(String prompt, boolean isEmpty, boolean isFirstPage) {
        synchronized (this) {
            mUiHandler.postDelayed(() -> {
                for (WeakReference<IBaseModelListener> weakListener : mWeakReferenceDeque) {
                    if (weakListener.get() instanceof IPagingModelListener) {
                        IPagingModelListener iModelListener = (IPagingModelListener) weakListener.get();
                        if (null != iModelListener) {
                            iModelListener.onLoadFail(BasePagingModel.this, prompt, isFirstPage);
                        }
                    }
                }
            }, 0);
